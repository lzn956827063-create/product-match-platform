...
def revision_quality(s,c,revision):
    ...
    descendants={}
    for link in s.scalars(select(RevisionLineage).join(Revision,(Revision.id==RevisionLineage.revision_id)&(Revision.org_id==RevisionLineage.org_id)).where(RevisionLineage.org_id==c.org_id,Revision.batch_id==revision.batch_id)):
        descendants.setdefault(link.parent_revision_id,[]).append(link.revision_id)
    visited=set();queue=[(revision.id,0)];depth=0
    while queue:
        parent,level=queue.pop()
        for child in descendants.get(parent,[]):
            if child not in visited:visited.add(child);queue.append((child,level+1));depth=max(depth,level+1)
