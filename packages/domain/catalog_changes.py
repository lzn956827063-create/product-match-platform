...
def create_change(s,c,from_id,to_id,links):
    ...
    products_before={p.id:p for p in s.scalars(select(Product).where(Product.org_id==c.org_id,Product.version_id==from_id))}
    products_after={p.id:p for p in s.scalars(select(Product).where(Product.org_id==c.org_id,Product.version_id==to_id))}
    require(set(links)<=products_before.keys() and set(links.values())<=products_after.keys() and len(set(links.values()))==len(links),422,'INVALID_IDENTITY_LINKS','商品身份对应必须是一对一且属于所选版本')
    # Persist the request; identity writes and impact analysis run asynchronously.
    obj=CatalogChange(id=uid(),org_id=c.org_id,catalog_id=before.catalog_id,from_version_id=from_id,to_version_id=to_id,links=links,created_by=c.user_id)
