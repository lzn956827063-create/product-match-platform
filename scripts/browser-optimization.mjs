...
const output=path.join(root,'docs/optimization/browser');fs.mkdirSync(output,{recursive:true})
...
try{
 ...
 await page.getByText('补数版本：仅含 1 条修正记录。全量原始记录请查看原运行。',{exact:true}).waitFor();await page.locator('.review-grid').waitFor()
 ...
 await page.getByRole('button',{name:taskName,exact:true}).first().click();await page.locator('.review-grid').waitFor()
 // Open the specific correction run via task list, which is sorted newest first.
 await page.getByText('补数版本：仅含 1 条修正记录。全量原始记录请查看原运行。',{exact:true}).waitFor()
