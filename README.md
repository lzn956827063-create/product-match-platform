...
根据 2026 年 9 月 10 日的产品方案和技术方案实现的可运行项目。将供应商 CSV / XLSX 映射到已发布的手机标准商品库，经人工审核后导出可追溯结果。

已提供前后端、数据库迁移、独立后台执行、离线 LightGBM 训练、评测、自动化测试和 Docker Compose。当前交付默认使用规则匹配；训练模型已登记为实验版本，尚未达到手机业务的模型准入条件。

![任务工作台](docs/dashboard.png)
...
6. 修订来源、更新标准库、切换方案均创建新运行，可比较同一批次的变化并查看原运行清单。
...
| `db/migrations` | Alembic 初始迁移，复合外键与有效映射唯一索引 |
